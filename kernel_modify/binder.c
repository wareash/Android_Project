			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	/* print write data */
	printk("%s, (%d, %d), %s, print data: \n",proc->tsk->comm, proc->tsk->pid, thread->pid, __FUNCTION__);	
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		if (_IOC_NR(cmd) < ARRAY_SIZE(binder_stats.bc)) {
			...
			thread->stats.bc[_IOC_NR(cmd)]++;
               ...
               /* process info: process name, process id, thread id, cmd name */
               printk("%s, (%d, %d), %s, %s \n",proc->tsk->comm, proc->tsk->pid, thread->pid, __FUNCTION__, binder_cmd_name(cmd));
			      ...
			      signed long *consumed, int non_block)
{
	...
	if (*consumed == 0) {
		...
		/* process info: process name, process id, thread id, cmd name */
		printk("%s, (%d, %d), %s, %s \n",proc->tsk->comm, proc->tsk->pid, thread->pid, __FUNCTION__, binder_cmd_name(BR_NOOP));
	...
	*consumed = ptr - buffer;
	if (proc->requested_threads + proc->ready_threads == 0 &&
		...
		if (put_user(BR_SPAWN_LOOPER, (uint32_t __user *)buffer))
			return -EFAULT;
		/* process info: process name, process id, thread id, cmd name */
		printk("%s, (%d, %d), %s, %s \n",proc->tsk->comm, proc->tsk->pid, thread->pid, __FUNCTION__, binder_cmd_name(BR_SPAWN_LOOPER));
